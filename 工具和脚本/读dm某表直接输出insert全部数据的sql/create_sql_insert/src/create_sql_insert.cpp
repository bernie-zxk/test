...
int DCI_SQL_TOOL::create_insert(char * tbname, char* outputfile)
{
	...
	int field_num;//未出错的字段的个数。实际输出sql的字段数
...
//	    bind_address = new _list_bind;
	...
	//cout<<"table  :"<<v_sqls.size()<<endl;
	osqlf<<"--   table  "<<tbname<<"   record number:"<<v_sqls.size()<<"    begin"<<endl;
	...
	for(i=0;i<v_sqls.size();i++)
	{
       ...
       osqlf<<"--   table  "<<tbname<<"   record number:"<<v_sqls.size()<<"    end"<<endl;

	osqlf.close();

	cout<<"output  succeeded  .   result  file   :  "<<outputfile<<endl;
